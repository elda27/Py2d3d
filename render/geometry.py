...
class GeometryContext:
    ...
    def view_matrix(self):
        if self.view_matrix_ is None:
            self.view_matrix = np.eye(4, 4, dtype=np.float32)
            self.view_matrix_[:3, :3] = np.matmul(
                amath.roty(self.cra),
                amath.rotx(self.rao)
            )
    ...
    def extrinsic(self):
        if self.extrinsic_ is None:
            extrinsic_T = amath.convertTransRotTo4x4(
                [0, 0, -self.SOD, 0, 0, 0])
            self.extrinsic_ = amath.concatenate4x4(
                extrinsic_T, self.view_matrix)
    ...
    def projection_matrix(self):
        if self.projection_matrix_ is None:
            self.projection_matrix_ = amath.constructProjectionMatrix(
